...
TEST_CASE("nr2i32(...), x = 0 => nr2i32(x) == 0, x = 2^n => nr2i32(x) == 2^n", "[sys][nr2i32]")
{
    CHECK(sys::nr2i32(0_i32) == 0_i32);
...
TEST_CASE("nr2i32(...), 2^n < x < 2^(n + 1) => nr2i32(x) == 2^(n + 1)", "[sys][nr2i32]")
{
...
TEST_CASE("nr2i64(...), x = 0 => nr2i64(x) == 0, x = 2^n => nr2i64(x) == 2^n", "[sys][nr2i64]")
{
    CHECK(sys::nr2i64(0_i64) == 0_i64);
...
TEST_CASE("nr2i64(...), 2^n < x < 2^(n + 1) => nr2i64(x) == 2^(n + 1)", "[sys][nr2i64]")
{
