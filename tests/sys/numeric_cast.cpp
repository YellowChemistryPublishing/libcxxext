...
TEST_CASE("Checking casts work as expected.", "[sys][numeric_cast]")
{
    ...
    {
        auto res1 = sys::bnumeric_cast<int32_t>(10); // NOLINT(readability-magic-numbers)
        CHECK(res1.expect() == 10);

        auto res2 = sys::bnumeric_cast<uint8_t>(255); // NOLINT(readability-magic-numbers)
        CHECK(res2.expect() == 255);
    }

    SECTION("Casting out of range returns empty result.")
    {
        auto res1 = sys::bnumeric_cast<uint8_t>(300); // NOLINT(readability-magic-numbers)
        CHECK_FALSE(res1);

        auto res2 = sys::bnumeric_cast<uint8_t>(-1); // NOLINT(readability-magic-numbers)
        CHECK_FALSE(res2);

        auto res3 = sys::bnumeric_cast<int8_t>(128); // NOLINT(readability-magic-numbers)
        CHECK_FALSE(res3);
    }
}
